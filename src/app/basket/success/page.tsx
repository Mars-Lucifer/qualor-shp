...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4 text-base font-medium">
      <span className="text-[#7E8395] shrink-0">{label}</span>
      <span className="text-[#1F2128] text-right">{value}</span>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-white font-[Inter,sans-serif] flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-4 sm:px-6 py-10 sm:py-16">
        <div className="w-full max-w-[650px]">
          <div className="bg-[#F5F5F5] rounded-[32px] p-8 sm:p-10 flex flex-col gap-8 sm:gap-10 items-center">
            <h1 className="text-[#1F2128] text-[36px] sm:text-[48px] font-medium leading-[1.08] self-start">
              🎉 Успешно!
            </h1>

            {/* Order details */}
            <div className="w-full rounded-[20px] border border-[#D6D6DB] p-5 flex flex-col gap-5">
            ...
            {ORDER_DETAILS.items.map((item) => (
              <div key={item} className="w-full rounded-[20px] border border-[#D6D6DB] p-5">
                <p className="text-[#1F2128] text-[18px] font-medium leading-[1.08]">{item}</p>
              </div>
            ))}
